...
<div id="profileModal" class="fixed inset-0 bg-black/40 hidden items-center justify-center">
  <div class="bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-xl p-6 w-[400px]">
    <h2 class="text-xl font-semibold mb-4">Ubah Profil</h2>

    <form method="POST" action="index.php?page=settings&action=update">
      <div class="mb-4">
        <label class="text-sm dark:text-slate-300">Username</label>
        <input name="username" value="<?= htmlspecialchars($username) ?>"
          class="w-full border border-slate-300 dark:border-slate-600 rounded px-3 py-2 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100">
      </div>

      <div class="mb-4">
        <label class="text-sm dark:text-slate-300">Email</label>
